...
function two({run, js, cost = 3, normalize}: {
    ...
    return class Two implements Fun {
        ...
        constructor(public a: Fun, public b: Fun) {}
        run() {
...
export type FunBatchResult = {done: boolean, failed: number, results: FunResult[]};

export class FunctionFinder {
    private inputFactories: InputFunFactory[];
    private funs: Generator<Fun, void, undefined>;

    constructor(private cases: Case[], maxDepth = 3) {
    ...
    batch(count = 50000): FunBatchResult {
        ...
        return {done, results, failed: totalFailed};
    }
}
...
export async function findFunctions(cases: Case[], maxDepth = 3) {
    ...
    function getSome() {
        const {done, results, failed} = finder.batch();
...
export type WorkerMessage = {type: 'start', cases: Case[], maxDepth: number} | {type: 'batch', count: number};
export type WorkerResponse = {type: 'started', error?: string} | ({type: 'result'} & FunBatchResult);
...
if (typeof self.WorkerGlobalScope !== 'undefined' && self instanceof self.WorkerGlobalScope) {
    ...
    self.addEventListener('message', ev => {
        switch (ev.data.type) {
            ...
            case 'batch': {
                const {done, results, failed} = finder?.batch(ev.data.count);
                self.postMessage({
                    type: 'batch',
                    done,
